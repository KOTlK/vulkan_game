...
void camera2d_make(Vector2 position, float aspect, float size, Camera2D* cam);
...
int main(int argc, char** argv) {
    ...
    // auto table = hash_table_make<int, const char*>();

    // hash_table_add(&table, 10, "Hello");
    // hash_table_add(&table, 20, "World");

    // printf("%s.\n", table[10]);
    // printf("%s.\n", table[20]);

    // hash_table_remove(&table, 10);

    // printf("%d.\n", hash_table_contains(&table, 10));

    const char* name         = "Hello";

    GlassErrorCode err = glass_create_window(100, 100, WIDTH, HEIGHT, name, &WINDOW);
    if (err != GLASS_OK) {
    ...
    // camera2d_make(vector2_make(0, 0), aspect, 4, &CAMERA);
...
// void camera2d_update(Camera2D* cam) {
//     cam->left   = cam->position.x - cam->size.x;
//     cam->right  = cam->position.x + cam->size.x;
//     cam->top    = cam->position.y + cam->size.y;
//     cam->bottom = cam->position.y - cam->size.y;
